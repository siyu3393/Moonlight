...
void main() {
	...
	if (z0 == 1.0) {
		z0 = texelFetch(dhDepthTex0, texelCoord, 0).r;
	}
	if (z1 == 1.0) {
		z1 = texelFetch(dhDepthTex1, texelCoord, 0).r;
	}
#endif
...
#ifdef DISTANT_HORIZONS
		vec3 viewPos = (z0 == 1.0) ? ScreenToView_DH(screenPos.xyz) : ScreenToView(screenPos.xyz);
#else
		vec3 viewPos = ScreenToView(screenPos.xyz);
#endif
...
#ifdef DISTANT_HORIZONS
		vec3 viewPos1 = (z1 == 1.0) ? ScreenToView_DH(screenPos1.xyz) : ScreenToView(screenPos1.xyz);
#else
		vec3 viewPos1 = ScreenToView(screenPos1.xyz);
#endif
