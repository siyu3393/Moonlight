// Use Moonlight water implementation
vec4 colorP = color;
    ...
    #ifdef GBUFFERS_WATER
        translucentMultCalculated = true;
        translucentMult = vec4(0.7, 0.8, 1.0, 1.0);
    #endif

    #if defined GBUFFERS_WATER && defined DEFERRED_BUFFER
        float normalStrength = WATER_BUMPINESS * 0.5;
        float normalClamp = 0.05;
        vec3 normalTex = vec3(sin(frameTimeCounter * 0.5) * 0.5 + 0.5, cos(frameTimeCounter * 0.5) * 0.5 + 0.5, 0.0);
        vec3 normalMap = vec3(normalTex.xy * 2.0 - 1.0, 0.0);
        normalMap.xy *= normalStrength;
        normalMap.z = sqrt(1.0 - dot(normalMap.xy, normalMap.xy));
        normal = normalize(mix(normal, normalMap, normalClamp));
    #endif

    #ifdef DEFERRED_BUFFER
        #if WATER_STYLE_DEFINE == 3
...
#define PHYSICS_OCEAN_INJECTION
#if defined GENERATED_NORMALS && (WATER_STYLE_DEFINE >= 2 || defined PHYSICS_OCEAN)
    ...
    #if WATER_QUALITY >= 2 || WATER_STYLE_DEFINE >= 2
    ...
    // Water Normals
    #if WATER_STYLE_DEFINE >= 2 || RAIN_PUDDLES >= 1 && WATER_STYLE_DEFINE == 1 && WATER_QUALITY >= 2
        vec3 normalMap = vec3(0.0, 0.0, 1.0);
        #if WATER_STYLE_DEFINE >= 2
        ...
        if (isEyeInWater != 1) {
            ...
            // Water Foam
                ...
                if (NdotU > 0.99) {
                    ...
                    #if WATER_STYLE_DEFINE < 3 && MC_VERSION >= 11300
            ...
            #if MC_VERSION < 11300 && WATER_STYLE_DEFINE >= 3
                color.a = 0.7;
            #endif
    ...
    #if WATER_STYLE_DEFINE == 3 || WATER_STYLE_DEFINE == 2 && SUN_MOON_STYLE_DEFINE >= 2
        smoothnessG = 1.0;
